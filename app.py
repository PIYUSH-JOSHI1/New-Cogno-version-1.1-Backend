...
def test_email():
    ...
    subject = "Cogno Solution - Email Test"
    body = """
    <h2>Email Configuration Successful!</h2>
    <p>This is a test email from Cogno Solution to verify your SMTP settings.</p>
    <p>If you received this, your email system is properly integrated.</p>
    <br>
    <p>Regards,<br>Cogno Solution Team</p>
    """
    
    return jsonify(send_email(email, subject, body))

@app.route('/api/email/login-notification', methods=['POST'])
@require_auth
def email_login_notification():
    """Send login notification email."""
    data = request.json
    user_name = data.get('user_name', 'Student')
    user_email = data.get('email')
    device_info = data.get('device_info', 'A new browser')
    
    if not user_email:
        user_email = request.user.get('email') or request.user.email
        
    body = email_templates.get_login_email(user_name, device_info)
    return jsonify(send_email(user_email, "New Login Detected - Cogno Solution", body))

@app.route('/api/email/achievement', methods=['POST'])
@require_auth
def email_achievement():
    """Send achievement notification."""
    data = request.json
    user_name = data.get('user_name', 'Student')
    user_email = data.get('email')
    title = data.get('title', 'Special Milestone')
    desc = data.get('description', 'You achieved a great milestone!')
    
    if not user_email:
        user_email = request.user.get('email') or request.user.email
        
    body = email_templates.get_achievement_email(user_name, title, desc)
    return jsonify(send_email(user_email, f"🏆 Achievement Unlocked: {title}", body))

@app.route('/api/email/consultation-reminder', methods=['POST'])
@require_auth
def email_consultation_reminder():
    """Send consultation reminder."""
    data = request.json
    user_name = data.get('user_name', 'Student')
    user_email = data.get('email')
    doctor_name = data.get('doctor_name', 'Specialist')
    date_time = data.get('date_time', 'Tomorrow')
    
    if not user_email:
        return jsonify({'error': 'Recipient email required'}), 400
        
    body = email_templates.get_appointment_reminder_email(user_name, doctor_name, date_time)
    return jsonify(send_email(user_email, "Reminder: Your Consultation Tomorrow", body))
...
def generate_progress_report():
    ...
    try:
        ...
        if request.args.get('email') == 'true' or (request.method == 'POST' and request.json.get('send_email')):
            recipient_email = request.args.get('recipient_email') or (request.json.get('email') if request.method == 'POST' else None) or user_data.get('email')
            if recipient_email:
                email_body = email_templates.get_report_email(
                    user_data.get('full_name', 'Student'),
                    report['summary']
                )
                email_result = send_email(
                    to=recipient_email,
                    subject=f"Cogno Solution - Progress Report for {user_data.get('full_name', 'Student')}",
                    body=email_body
                )
                response_data['email_sent'] = email_result['success']
                response_data['email_message'] = email_result.get('message') or email_result.get('error')
...
@app.route('/api/admin/process-reminders', methods=['GET', 'POST'])
def process_reminders():
    """
    Check for upcoming appointments and send reminders.
    Usually triggered by a cron job/webhook.
    """
    if not supabase:
        return jsonify({'error': 'Supabase not configured'}), 500
        
    try:
        # Get appointments for the next 48 hours that haven't been reminded
        now = datetime.utcnow()
        limit = now + timedelta(hours=48)
        
        # This is a simplified logic. In a real app, you'd track 'reminder_sent' in the DB.
        result = supabase.table('consultations').select('*, patient:patient_id(*), doctor:doctor_id(*)').gte('scheduled_at', now.isoformat()).lte('scheduled_at', limit.isoformat()).execute()
        
        count = 0
        for consult in result.data:
            patient = consult.get('patient', {})
            doctor = consult.get('doctor', {})
            
            if patient.get('email'):
                body = email_templates.get_appointment_reminder_email(
                    patient.get('full_name', 'Student'),
                    doctor.get('full_name', 'Specialist'),
                    datetime.fromisoformat(consult['scheduled_at'].replace('Z', '+00:00')).strftime('%B %d, %Y at %H:%M')
                )
                send_email(patient['email'], "Reminder: Your Consultation Tomorrow", body)
                count += 1
